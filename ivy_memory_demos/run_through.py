...
def main():
    ...
    class TorchModelWithLSTM(torch.nn.Module):
        def __init__(self, channels_in, channels_out):
            ...
            self._lstm = ivy_mem.LSTM(
                64, channels_out, num_layers=2, return_state=False
            )
    ...
    batch_shape = [1, 2]
    timesteps = 3
    batch_shape + [timesteps, in_channels]
    ...
    class TfModelWithNTM(tf.keras.Model):
        ...
        def _assign_variables(self):
            self._ntm.v.map(lambda x, kc: self.add_weight(name=kc, shape=x.shape))
    ...
    cam_coords = ivy_vision.ds_pixel_to_cam_coords(ds_pixel_coords, inv_calib_mats)[
        ..., 0:3
    ]
    features = ivy.random_uniform(
        shape=[batch_size, num_timesteps] + image_dims + [num_feature_channels]
    )
    ...
    esm_obs = ESMObservation(
        img_meas={"camera_0": esm_cam_meas}, agent_rel_mat=agent_rel_mat
    )
