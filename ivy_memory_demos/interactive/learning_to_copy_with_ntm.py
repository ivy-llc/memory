...
def loss_fn(ntm, v, total_seq, target_seq, seq_len):
    ...
    return ivy.sum(ivy.binary_cross_entropy(pred_vals, target_seq)) / pred_vals.shape[0], pred_vals
...
def main(batch_size=32, num_train_steps=31250, compile_flag=True,
    ...
    for i in range(num_train_steps):
        ...
        if i % vis_freq == 0:
            ...
            target_to_vis = np.transpose(cv2.resize(target_to_vis, (560, 160), interpolation=cv2.INTER_NEAREST), (1, 0))

            pred_to_vis = (ivy.to_numpy(pred_vals[0] * 255)).astype(np.uint8)
            pred_to_vis = np.transpose(cv2.resize(pred_to_vis, (560, 160), interpolation=cv2.INTER_NEAREST), (1, 0))

            img_to_vis = np.concatenate((pred_to_vis, target_to_vis), 0)
            img_to_vis = cv2.resize(img_to_vis, (1120, 640), interpolation=cv2.INTER_NEAREST)

            img_to_vis[0:60, -200:] = 0
            img_to_vis[5:55, -195:-5] = 255
            cv2.putText(img_to_vis, 'step {}'.format(i), (935, 42), cv2.FONT_HERSHEY_SIMPLEX, 1.2, tuple([0] * 3), 2)

            img_to_vis[0:60, 0:200] = 0
            img_to_vis[5:55, 5:195] = 255
            cv2.putText(img_to_vis, 'prediction', (7, 42), cv2.FONT_HERSHEY_SIMPLEX, 1.2, tuple([0] * 3), 2)

            img_to_vis[320:380, 0:130] = 0
            img_to_vis[325:375, 5:125] = 255
            cv2.putText(img_to_vis, 'target', (7, 362), cv2.FONT_HERSHEY_SIMPLEX, 1.2, tuple([0] * 3), 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_training_steps', type=int, default=31250,
                        help='The number of steps to train for.')
    parser.add_argument('--eager', action='store_true',
                        help='Whether to compile the training step.')

    parser.add_argument('--num_bits', type=int, default=8,
                        help='Number of bits in the NTM memory.')
    parser.add_argument('--seq_len', type=int, default=28,
                        help='Sequence length for the NTM memory.')
    parser.add_argument('--ctrl_output_size', type=int, default=100,
                        help='Output size from the NTM controller.')
    parser.add_argument('--memory_size', type=int, default=128,
                        help='NTM memory size.')
    parser.add_argument('--memory_vector_dim', type=int, default=28,
                        help='NTM memory vector dimension.')
