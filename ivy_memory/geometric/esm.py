...
class ESM(ivy.Module):
    ...
    def __init__(self, depth_prior=None, feat_prior=None, num_feature_channels=3,
                 ...
                 threshold_var_factor=0.99, depth_limits=(1e-3, 10.),
                 depth_var_limits=(1e-3, 1e4),
                 feat_var_limits=(1e-3, 1e4), omni_image_dims=(180, 360),
                 smooth_mean=True, depth_buffer=True,
    ...
    def _frame_to_omni_frame_projection(self, cam_rel_poses, cam_rel_mats,
                                        uniform_sphere_pixel_coords, cam_coords_f1,
                                        cam_feat_f1, rel_pose_covs, image_var_f1,
                                        holes_prior, holes_prior_var,
        ...
        angular_pixel_coords_f2 = \
            ivy_vision.sphere_to_angular_pixel_coords(sphere_coords_f2,
    ...
    def _omni_frame_to_omni_frame_projection(self, agent_rel_pose, agent_rel_mat,
                                             uniform_sphere_pixel_coords,
                                             sphere_pix_coords_f1, sphere_depth_f1,
                                             sphere_feat_f1, agent_rel_pose_cov,
                                             image_var_f1, holes_prior, holes_prior_var,
                                             batch_size):
        ...
        # B x OH x OW x 3
        sphere_coords_f1 = \
            ivy_vision.angular_pixel_to_sphere_coords(angular_pixel_coords_f1,
                                                      self._pixels_per_degree)
        ...
        projected_coords_f2_flat = ivy.reshape(projected_coords_f2,
                                               [batch_size] + [
                                                   self._sphere_img_dims[0] *
                                                   self._sphere_img_dims[1]]
                                               + [3 + self._feat_dim])

        # B x (OHxOW) x (3+F)
        image_var_f2_flat = ivy.reshape(image_var_f2,
                                        [batch_size] + [self._sphere_img_dims[0] *
                                                        self._sphere_img_dims[1]]
                                        + [3 + self._feat_dim])
    ...
    def _convert_images_to_omni_observations(self, measurements,
                                             uniform_sphere_pixel_coords,
                                             holes_prior, batch_size, num_timesteps,
                                             num_cams, image_dims):
        ...
        return self._frame_to_omni_frame_projection(
            ...
            images[..., 0:3], images[..., 3:],
            cam_to_cam_pose_covs, var_to_project, holes_prior, holes_prior_var,
            batch_size, num_timesteps, num_cams,
            image_dims)
    ...
    def _kalman_filter_on_measurement_sequence(self, prev_fused_val,
                                               prev_fused_variance, hole_prior,
                                               hole_prior_var, meas, meas_vars,
                                               uniform_sphere_pixel_coords,
                                               agent_rel_poses, agent_rel_pose_covs,
                                               agent_rel_mats,
                                               batch_size, num_timesteps):
        ...
        for i in range(num_timesteps):
            ...
            fused_val_unsmoothed, fused_variance_unsmoothed = \
                self._fuse_measurements_with_uncertainty(prior_and_meas,
                                                         prior_and_meas_variance, 1)

            # B x OH x OW x (3+F) This prevents accumulating certainty from duplicate
            # re-projections from prior measurements
            fused_variance_unsmoothed = ivy.where(low_var_mask,
                                                  fused_variance_unsmoothed[:, 0],
                                                  hole_prior_var)
            ...
            fused_val, fused_variance = self.smooth(
                fused_val, fused_variance, low_var_mask, self._smooth_mean,
    ...
    def smooth(self, fused_val, fused_variance, low_var_mask, smooth_mean,
        ...
        if smooth_mean:
            ...
            fused_variance_pad = ivy_vision.pad_omni_image(var_for_smoothing, pad_size,
                                                           self._sphere_img_dims)
            expanded_sphere_img_dims = [item + 2 * pad_size for item in
                                        self._sphere_img_dims]

            # reshape for dilation and erosion
            fused_val_pad_flat = ivy.reshape(fused_val_pad[..., 2:],
                                             [batch_size] + expanded_sphere_img_dims +
                                             [1 + self._feat_dim])
            fused_var_pad_flat = ivy.reshape(fused_variance_pad[..., 2:],
                                             [batch_size] + expanded_sphere_img_dims +
                                             [1 + self._feat_dim])

            if variance_mode:
                smoothed_fused_val_flat, smoothed_fused_var_flat = \
                    ivy_vision.smooth_image_fom_var_image(fused_val_pad_flat,
                                                          fused_var_pad_flat,
                                                          smooth_kernel_size,
                                                          ivy.array([1.] * (
                                                                      1 + self._feat_dim),
    ...
    def _forward(self, obs: ESMObservation, memory: ESMMemory = None, batch_size=None,
                 num_timesteps=None,
        ...
        var_threshold_max = ivy.tile(
            ivy.reshape(ivy.stack([self._ang_pix_var_threshold] * 2 +
        ...
        # B x N x OH x OW x 1
        fused_depth_variance = ivy.clip(fused_variance[..., 0:1], self._min_depth_var,
                                        self._depth_prior_var_val)

        # B x N x OH x OW x 3
        fused_feat_variance = ivy.clip(fused_variance[..., 1:], self._min_feat_var,
                                       self._feat_prior_var_val)
