...
def test_demo_ntm_copy(compile_flag, dev_str, fw):
    ...
    if fw in ["torch", "jax"] and compile_flag:
        ...
        pytest.skip()
    main(1, 1, compile_flag, 1, 1, 1, 4, 1, interactive=False, fw=fw)


@pytest.mark.parametrize("with_sim", [False])
@pytest.mark.parametrize("compile_flag", [True, False])
def test_demo_esm(with_sim, compile_flag, dev_str, fw):
