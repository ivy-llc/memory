...
def test_values(dev_str, fw):
    ...
    for i in range(2):
        obs = ivy.Container.cont_from_disk_as_hdf5(os.path.join(this_dir, 'test_data/obs_{}.hdf5'.format(i)))
        memory = esm(obs, memory, batch_size=batch_size, num_timesteps=num_timesteps, num_cams=num_cams,
                     image_dims=image_dims)
        expected_mem = ivy.Container.cont_from_disk_as_hdf5(os.path.join(this_dir, 'test_data/mem_{}.hdf5'.format(i)))
