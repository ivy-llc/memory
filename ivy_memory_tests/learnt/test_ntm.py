...
class NTMTestData:
    def __init__(self):
        ...
        self.ntm_return["content"] = np.array(
            [
                [
                    [
                        0.00869071,
        ...
        self.ntm_return["content_and_location"] = np.array(
            [
                [
                    [
                        0.00869071,
...
def test_ntm(addressing_mode, batch_shape, dev_str, f, fw):
    ...
    variables["ntm_cell"]["controller"] = {
        "input": {
            "layer_0": {
                "w": ivy.array(
                    np.random.uniform(
                        -in_wlim, in_wlim, size=[ctrl_input_size, 4 * ctrl_output_size]
                    ).astype(np.float32)
                )
            },
            "layer_1": {
                "w": ivy.array(
                    np.random.uniform(
                        -in_wlim, in_wlim, size=[ctrl_output_size, 4 * ctrl_output_size]
                    ).astype(np.float32)
                )
            },
        },
            ...
            "layer_0": {
                "w": ivy.array(
                    np.random.uniform(
                        -rec_wlim,
                        rec_wlim,
                        size=[ctrl_output_size, 4 * ctrl_output_size],
                    ).astype(np.float32)
                )
            },
            "layer_1": {
    ...
    variables["ntm_cell"]["controller_proj"] = {
        "w": ivy.array(
            np.random.uniform(
                -proj_wlim, proj_wlim, size=[total_parameter_num, ctrl_output_size]
            ).astype(np.float32)
        ),
    ...
    variables["ntm_cell"]["output_proj"] = {
        "w": ivy.array(
            np.random.uniform(
                -out_wlim,
                out_wlim,
                size=[output_dim, ctrl_output_size + read_head_num * memory_vector_dim],
            ).astype(np.float32)
        ),
